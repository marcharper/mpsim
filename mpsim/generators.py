...
def iterations_generator(total, per_run=1000):
    """
...
def parameter_generator(cache, initial_state_gen, seed_gen=None,
    ...
    max_iterations_gen = constant_generator(max_iterations)
    if not seed_gen:
        seed_gen = random_seed_generator()
    return itertools.izip(cache_gen, initial_state_gen, seed_gen, max_iterations_gen)
